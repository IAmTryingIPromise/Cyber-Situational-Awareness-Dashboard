...
from .schemas import (
    ...
    AssetInfo, CVEInfo, CWEInfo, CAPECInfo, AttackInfo, GetOSFamiliesRequest,
    OSFamiliesResponse, ScanByOSRequest
)

# Configure scanner
PROJECT_ROOT = Path(__file__).parent
DATA_FILE_ROOT = PROJECT_ROOT.parent
...
async def search_cpe_matches(request: CPESearchRequest):
    ...
    try:
        ...
        logger.info(f"Device Search: Response time: {elapsed_time:.4f}s ({elapsed_time * 1000:.2f}ms)")
        ...
        logger.error(f"⏱️  Failed after: {elapsed_time:.4f}s ({elapsed_time * 1000:.2f}ms)")
...
@router.post("/get-os-families", response_model=OSFamiliesResponse)
async def get_os_families(request: GetOSFamiliesRequest):
    """
    API 2: Get OS families for a specific vendor
    """

    start_time = time.perf_counter()

    try:
        logger.info(f"Getting OS families for vendor: {request.vendor}")
        
        # Load OS families data
        os_families_path = str(DATA_FILE_ROOT / "data" / "osfamilies.json")
        with open(os_families_path, 'r', encoding='utf-8') as f:
            os_families_data = json.load(f)
        
        # Find vendor (case insensitive)
        vendor_key = None
        for key in os_families_data.keys():
            if key.lower() == request.vendor.lower():
                vendor_key = key
                break
        ...
        vendor_data = os_families_data[vendor_key]

        elapsed_time = time.perf_counter() - start_time
        logger.info(f"OS Families: Response time: {elapsed_time:.4f}s ({elapsed_time * 1000:.2f}ms)")
        
        return OSFamiliesResponse(
            vendor=vendor_key,
            os_families=vendor_data["osFamilies"],
            default_os_family=vendor_data["default"],
            urls=vendor_data["urls"]
        )
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error getting OS families for {request.vendor}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error retrieving OS families: {str(e)}")


@router.post("/scan-by-os", response_model=FullScanResponse)
async def scan_device_by_os(request: ScanByOSRequest, db: Session = Depends(get_db)):
    """
    API 3: Scan device by OS family and version
    """

    start_time = time.perf_counter()
    
    try:
        logger.info(f"Starting OS-based scan for: {request.device_name}")
        logger.info(f"Hardware CPE: {request.h_cpe}")
        logger.info(f"OS: {request.vendor} {request.os_family} {request.version}")
        
        # Run the scan using the new OS-based method
        scan_results = await scanner.scan_device_by_os(
            device_name=request.device_name,
            h_cpe=request.h_cpe,
        ...
        stmt = select(Asset).where(Asset.name == request.device_name)
        asset = db.scalar(stmt)

        elapsed_time = time.perf_counter() - start_time
        
        if not asset:
            # Return success but no vulnerabilities found
            return FullScanResponse(
                success=True, error_message=None, scan_time=elapsed_time,
                device=None, cves=[], cwes=[], capecs=[], attacks=[],
                statistics={"cves": 0, "cwes": 0, "capecs": 0, "attacks": 0}
            )
        
        # Get all related data and build response
        response_data = _build_full_scan_response(db, asset, elapsed_time)

        elapsed_time = time.perf_counter() - start_time
        logger.info(f"Initial CVE Scan: Response time: {elapsed_time:.4f}s ({elapsed_time * 1000:.2f}ms)")

        return response_data
    
    except ValueError as ve:
        elapsed_time = time.perf_counter() - start_time
        # Handle CPE validation errors specifically
        logger.error(f"CPE validation error: {str(ve)}")
        return FullScanResponse(
            success=False, 
            error_message=f"Invalid CPE format: {str(ve)}", 
            scan_time=elapsed_time,
            device=None, cves=[], cwes=[], capecs=[], attacks=[],
            statistics={"cves": 0, "cwes": 0, "capecs": 0, "attacks": 0}
        )
        ...
        elapsed_time = time.perf_counter() - start_time
        logger.error(f"OS-based scan failed: {str(e)}")
        return FullScanResponse(
            success=False, error_message=str(e), scan_time=elapsed_time,
            device=None, cves=[], cwes=[], capecs=[], attacks=[],
            statistics={"cves": 0, "cwes": 0, "capecs": 0, "attacks": 0}
        )
...
async def delete_device(asset_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not asset:
            elapsed_time = time.perf_counter() - start_time
            logger.info(f"Deleting Asset: Response time: {elapsed_time:.4f}s ({elapsed_time * 1000:.2f}ms)")
            raise HTTPException(status_code=404, detail="Device not found")
